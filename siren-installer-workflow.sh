...
#  RackunSec
#  Version 3.4.17 - for siren LINUX
...
mv /usr/share/applications/siren-installer.desktop /tmp
...
export OS="siren Linux 3.4"
export WORKINGDIR=/mnt/siren
export LOCALICONS=/usr/share/siren/images/icons/
export TITLETEXT="siren Linux - Advanced Live Installer"
export WINDOWICON="${LOCALICONS}siren-64-white.png"
export WINDOWIMAGE="${LOCALICONS}siren-install-icon.png"
export SECICON="${LOCALICONS}sec-installer.png"
export GEARICON="${LOCALICONS}siren-config-small.png"
...
export TITLETEXT='"siren Linux - HDD Installation Tool"'
...
HTTPTEST=$(curl -sIL https://pornhub.com/index.php | head -n 1 | awk '{print $2}')
if [ "$HTTPTEST" != "200" ];
 then
...
if [ $ans = 1 ]; then
 ...
 firefox-esr 'https://sirenlinux.com/tutorials.php' &
fi
...
if [ "$DISKENC" == "1" ]; then
  ...
  tilix -e "/bin/bash -c \"while ! cryptsetup luksOpen /dev/$TARGETPART sirenluks;do echo 'Bad passphrase or disk not found ... '; done\""
  while [ "$(ps aux | egrep 'while...cryptsetup.luks[O]'|wc -l)" == "1" ]
  do # sleep until this process is done:
    sleep 1
  done
  printf "[log] Creating mapper objects ... "
  pvcreate /dev/mapper/sirenluks
  vgcreate cryptvg /dev/mapper/sirenluks
  ...
  then
    ...
    sleep 1
    mkdir /mnt/siren 2>/dev/null
    mount /dev/mapper/cryptvg-lvroot /mnt/siren
  fi
else
  # Simply mount /dev/$PARTINST to /mnt/siren
  printf "[log] Mounting /dev/$TARGETPART to /mnt/siren"
  mkdir /mnt/siren 2>/dev/null
  mount /dev/$TARGETPART /mnt/siren
fi
...
KERNEL=$(yad --list --column="New Kernel" $KERNELS \
  --width=500 --height=300 --window-icon=64-icon.png\
   --text="\nPlease choose a kernel to use on your installed siren.\n"\
    --image=${WINDOWIMAGE})
...
cat /usr/share/zoneinfo/zone.tab | awk '{print $3}' | grep "/" | sort > /tmp/siren-installer-script.zoneinfo
for i in `cat /tmp/siren-installer-script.zoneinfo`; do
  ZONES="$ZONES $i Timezone"
done
rm /tmp/siren-installer-script.zoneinfo
ZONESINFO=""
while [ "$ZONESINFO" = "" ]
do
...
progressBar "Copying the files to <b>/dev/$TARGETPART</b>.   \nThis <u>will</u> take a long time - ($(df -h 2>/dev/null | awk '$1 ~ /overlay|sr0|loop/ {gsub(/[.,]/,"",$2); gsub(/G/,"00",$2); gsub(/M/,"",$2); size+=$2}END{print size}')MB).   " &
  ...
  rsync -a / $WORKINGDIR --ignore-existing --exclude=/{lib/live,usr/lib/live,live,cdrom,mnt,proc,run,sys,media,appdev,siren-dev,tmp}
  ...
  cp /usr/share/siren/files/lightdm-gtk-greeter.conf $WORKINGDIR/etc/lightdm/ # copy the config file over to new installation
  if [[ "$DISKENC" == "1" ]]
  then
    # get UUID of disk
    export DISKUUID=$(blkid | grep crypto_LUKS|awk -F\" '{print $2}')
    echo "sirenluks UUID=$DISKUUID  none  luks" >> ${WORKINGDIR}/etc/crypttab
    ...
    cp ${WORKINGDIR}/sbin/e2fsck ${WORKINGDIR}/mnt/siren/sbin/fsck.luks
...
GRUB
  ...
  echo "GRUB_BACKGROUND=\"/usr/share/siren/images/splash.png\"" >> $WORKINGDIR/etc/default/grub
...
ans=$?
if [ $ans = 0 ]; then
 $DIALOG $TITLE"$TITLETEXT" $MSGBOX $TEXT"$SPANFONT Thank you for choosing <b>siren Linux.</b>   \n\n~Douglas Berdeaux\nWeakNetLabs@Gmail.com</span>" --image=$WINDOWIMAGE --window-image=$WINDOWIMAGE;
...
mv /tmp/siren-installer.desktop /usr/share/applications/
exit 0;
